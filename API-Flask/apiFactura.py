...
class Factura(Resource):
    def post(self):
        ...
        # Solicitud a la API de Productos (para obtener el precio del producto)
            producto_response = requests.get(f"{url_producto}/{json['producto_id']}")
        ...
        #Actualizar datos de boleta
                        ...
                        requests.put(f"{url_logistica}/{json['producto_id']}",
                        json={'stock': logistica_json['stock'] - json['cantidad']})
                        return objRespuesta
                    else:
